...
| `POST /token`                                                      | OAuth2 refresh-token grant (stub)      | No                                  |
...
**Stubbed refresh token.** The token endpoint's JSON includes a `refresh_token` field carrying a **stubbed** refresh token: a crypto-random value minted per proxy instance — it is not a GCP credential, is never sent to the gate, and the proxy itself keeps it in memory only. Processes that obtained it (i.e., that authenticated through the token endpoint — `Metadata-Flavor: Google` header, plus PID validation on `with-prod` temporary proxies) can drive a standard OAuth2 refresh flow against `POST /token` (emulating `https://oauth2.googleapis.com/token`, form-encoded `grant_type=refresh_token&refresh_token=<stub>`, body capped at 8 KB), which returns the current short-lived access token. Redeeming the stub goes through the proxy's normal token provider, so every gate-side bound — session expiry, token TTL, confirmation policy — still applies; the stub cannot extend a prod session. Exfiltrating it is useless outside the container: it is only honored by the issuing proxy on `127.0.0.1` and dies with the proxy process. The real refresh credentials never enter the container (the engineer's ADC refresh token stays in the host-side gate; the prod session ID stays in the `with-prod` parent process) — the only GCP credential a compromised container process can steal remains the temporary OAuth access token. google-auth for Python and Go, gcloud, and `gcp-metadata` ignore the extra `refresh_token` field; Node's `google-auth-library` is the exception — its Compute client copies the full token JSON into `client.credentials` and re-emits it on the `'tokens'` event, so an app that persists tokens through that hook will write the stub to disk alongside the (equally sensitive, shorter-lived) access token, where it stays redeemable at the issuing proxy's `POST /token` until that proxy exits. Like Google's endpoint, refresh responses carry `Cache-Control: no-store`, do not include a new `refresh_token`, and errors use RFC 6749 codes (`invalid_grant`, `invalid_request`, `unsupported_grant_type`; `temporarily_unavailable` with HTTP 503 when the gate is unreachable).
...
- **Refresh tokens in the container are stubs.** The `refresh_token` the metadata proxy hands out is minted by the proxy itself (crypto-random; the proxy stores it in memory only, though a client app that persists the token JSON it received — e.g. Node `google-auth-library`'s `'tokens'` event — may write its copy to disk) and is only honored by that proxy instance's `POST /token` endpoint on `127.0.0.1`. It is not a GCP credential: redeeming it just returns the proxy's current short-lived access token, still bounded by gate-enforced session expiry and token TTL. The engineer's real ADC refresh token never leaves the host, and the prod session ID never leaves the `with-prod` parent process, so nothing exfiltratable from the container can refresh or extend access from outside it — the only stealable GCP credential is the temporary OAuth access token.
