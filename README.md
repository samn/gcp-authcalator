...
**Stubbed refresh token.** The token endpoint's JSON includes a `refresh_token` field carrying a **stubbed** refresh token: a crypto-random value minted per proxy instance and held only in that proxy's memory — it is not a GCP credential and is never sent to the gate. Processes that obtained it (i.e., that authenticated through the token endpoint — `Metadata-Flavor: Google` header, plus PID validation on `with-prod` temporary proxies) can drive a standard OAuth2 refresh flow against `POST /token` (emulating `https://oauth2.googleapis.com/token`, form-encoded `grant_type=refresh_token&refresh_token=<stub>`), which returns the current short-lived access token. Redeeming the stub goes through the proxy's normal token provider, so every gate-side bound — session expiry, token TTL, confirmation policy — still applies; the stub cannot extend a prod session. Exfiltrating it is useless: it is only honored by the issuing proxy on `127.0.0.1` and dies with the proxy process. The real refresh credentials never enter the container (the engineer's ADC refresh token stays in the host-side gate; the prod session ID stays in the `with-prod` parent process) — the only GCP credential a compromised container process can steal remains the temporary OAuth access token. Known metadata clients (google-auth for Python/Node/Go, gcloud, `gcp-metadata`) ignore the extra `refresh_token` field, and real GCE metadata consumers are unaffected. Like Google's endpoint, refresh responses do not include a new `refresh_token`, and invalid grants return RFC 6749-style errors (`invalid_grant`, `invalid_request`, `unsupported_grant_type`).
