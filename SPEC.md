...
- **Non-mutating nested detection** — validates the parent proxy through its root, email, and project routes without calling `/token`; a health check therefore cannot trigger a long PAM refresh that it will abandon after its short probe deadline
