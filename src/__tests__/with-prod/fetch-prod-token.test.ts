...
function mockFetchText(text: string, status: number): typeof globalThis.fetch {
  return (async () =>
    new Response(text, { status })) as unknown as typeof globalThis.fetch;
}
...
describe("fetchProdToken", () => {
  test("fetches with ?level=prod and unix socket option", async () => {
    ...
    const fetchFn = (async (url: string, init: Record<string, unknown>) => {
      ...
      capturedInit = init;
      return new Response(
        JSON.stringify({ access_token: "prod-tok-123", expires_in: 1800 }),
  ...
  test("throws on non-OK response", async () => {
    ...
    await expect(
      fetchProdToken("/tmp/gate.sock", { fetchFn }),
