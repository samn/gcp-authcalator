...
| Auto-regeneration     | No; explicit via `init-tls` only                                          | Prevents silent CA changes that invalidate exported client bundles                                                                                                        |
...
1. Gate loads a private CA generated by `gcp-authcalator init-tls` (gate never generates certs itself)
...
- Gate does **not** auto-regenerate certs — it loads existing certs and fails with a clear error if they are missing or expired, directing the user to run `gcp-authcalator init-tls`
- 90-day client/server certs → ~4 rotations per year
- All certs (CA, server, client) share the same 90-day lifetime and are regenerated together
- Rotation is manual: run `init-tls`, then update the client bundle in remote environments
...
- Functions:
  - `ensureTlsFiles(tlsDir?, force?)`: Generate all certs if missing or expired; return paths. Used only by `init-tls`.
  - `loadAndValidateTlsFiles(tlsDir?)`: Load existing certs and validate they are not expired. Used by gate server. Throws actionable errors if certs are missing or expired.
...
- Gate loads and validates existing certs (via `loadAndValidateTlsFiles()`); fails if missing or expired
...
- **Certificate expiry**: Gate refuses to start with expired certs, directing the user to run `init-tls`. This prevents silent CA changes that would invalidate exported client bundles.
